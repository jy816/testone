...
{
    int 			i=0,j=0,ret_value;	/*循环变量i，j。返回值存储变量ret_value*/
    ...
    UINT8 			switchValue;
    int 			dataSum = 0;
    const float 	sampleFullRange[]={100,100,100,100,100,100,100,100,100,100,100,
											400,35,1.6,0.6,0.4,1.6,
											100,15000,100};/*11个温度   6个压强    3个其他  满量程	kPa,kPa,*/
            ...
            for(i=0;i<NUM_ADC_CHANNEL;i++)//采集10次之后，每个通道10个数去掉最大最小2个，然后求均值，存储到global全局变量
                {
					...
					globalADValue[i] = (float)dataSum/(NUM_SAMPLE_LOOP-4)/NUM_THOUSAND;//AD每个通道的模拟量    sum/6/16384
					tmp_AD =NUM_FOURTY* globalADValue[i]-20;//40*globalADValue-20
					/* CH0 压气机机匣温度*/
					globalADValue[i] = ((tmp_AD- NUM_FOUR)/NUM_SIXTY)*sampleFullRange[i];		/* 得到两个M35的值，(tmp_AD-4)/60*满量程 */
                }
               
			    value = mread(M31_BASE_ADDR)&0xffff;    //读M31模块基地址，M31模块用来输入数字量
                ...
                if(LOOP == 10)                    /*进程执行10次，100ms*10=1S，把数据放到共享内存*/
                {
